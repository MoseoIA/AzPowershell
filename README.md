...
Claro, puedo ayudarte a identificar y detallar los 5 principales riesgos asociados con la actualización de contraseñas de las cuentas de servicio en tu entorno de SharePoint Server 2013 on-premise, basándome en la información de las fuentes proporcionadas y considerando tu infraestructura de 3 servidores (uno con SQL+App Service, y dos con App+WFE). La actualización de estas contraseñas es una tarea crítica con desafíos significativos.

Aquí están los 5 riesgos principales, extraídos de las fuentes, al actualizar las contraseñas de las cuentas de servicio en un entorno como el tuyo:

*   **Desincronización de contraseñas entre Active Directory (AD) y SharePoint, llevando a fallos generalizados del servicio**: Este es el riesgo más directo y común. Las cuentas de servicio de SharePoint están profundamente integradas y un cambio de contraseña en AD no es simplemente una operación de AD, sino que afecta la operatividad de múltiples componentes de SharePoint. Si la nueva contraseña en AD no se actualiza correctamente en SharePoint, se produce una desincronización. Los servicios de SharePoint intentarán autenticarse con la contraseña antigua que ya no es válida en AD, lo que resultará en fallos. Esto puede afectar a **cualquier servicio, aplicación web o trabajo de temporizador que dependa de una cuenta de servicio cuya contraseña esté desincronizada**. Incluso la Administración Central puede verse afectada si su grupo de aplicaciones se ejecuta bajo una cuenta con una contraseña incorrecta. Este riesgo es inherente a cualquier granja, incluyendo la tuya, donde múltiples servicios y roles de servidor dependen de estas cuentas.

*   **Bloqueos de cuentas en Active Directory y problemas de acceso denegado**: Una consecuencia directa y peligrosa de la desincronización de contraseñas es que los componentes de SharePoint, al intentar autenticarse repetidamente con una contraseña antigua en varios servidores (tus 3 servidores en este caso), pueden **bloquear las cuentas de servicio en Active Directory**. Esto es especialmente probable si existen políticas de bloqueo de cuentas estrictas en tu dominio. Además, si las contraseñas de cuentas con privilegios administrativos, como la cuenta de la granja, son incorrectas, los administradores podrían enfrentarse a **errores de "Acceso Denegado"** al intentar acceder a la Administración Central o ejecutar comandos de PowerShell.

*   **Propagación incompleta de los cambios de contraseña, específicamente para la Cuenta de la Granja**: La Cuenta de la Granja (Farm Account) es una de las cuentas más críticas. Después de cambiar su contraseña en AD, **no basta con actualizarla en un solo lugar en SharePoint; debes ejecutar el comando `stsadm -o updatefarmcredentials` en *todos* los servidores de SharePoint en la granja**. En tu entorno de 3 servidores, esto significa ejecutar el comando en el servidor que hospeda SQL y App Service, y también en los otros dos servidores (App y WFE). Si omites algún servidor, este seguirá intentando usar la contraseña antigua, lo que llevará a estados inconsistentes y fallos en los servicios que se ejecutan en ese servidor específico. El hecho de que este comando deba ejecutarse por servidor resalta que este cambio de credencial específico no se propaga automáticamente a través de la base de datos de configuración de la granja.

*   **Fallo en servicios de aplicaciones específicos y funcionalidades clave como la Búsqueda o el Servicio de Perfiles de Usuario (UPS)**: Más allá de los fallos generales, ciertos servicios de aplicación tienen dependencias críticas en sus cuentas de servicio. Por ejemplo:
    *   Para la **Búsqueda**, las funciones de consulta y rastreo se verán afectadas si la contraseña de la cuenta del servicio de búsqueda o la cuenta de acceso a contenido no se actualizan. Las consultas pueden tener un tiempo de inactividad estimado de 3 a 5 minutos durante los reinicios del servicio, y los rastreos fallarán si la cuenta de acceso a contenido tiene una contraseña incorrecta.
    *   Para el **Servicio de Perfiles de Usuario (UPS)**, la sincronización de perfiles fallará si las contraseñas de la cuenta del servicio UPSS o la cuenta de conexión de sincronización no se actualizan. Esto impacta directamente características que dependen de perfiles actualizados.
    *   Otras aplicaciones de servicio como PerformancePoint Services, Excel Services, Visio Graphics Service, etc., también fallarán si las contraseñas de sus cuentas respectivas no se actualizan correctamente en sus configuraciones. La ubicación específica para actualizar estas contraseñas puede variar (página de configuración del servicio, cuentas administradas, o incluso directiva de aplicación web).

*   **Credenciales obsoletas en configuraciones fuera de la gestión centralizada de SharePoint, como grupos de aplicaciones de IIS, scripts personalizados o tareas programadas**: Mientras que las "cuentas administradas" de SharePoint buscan simplificar la gestión de contraseñas, no todas las cuentas o configuraciones pueden ser gestionadas por este mecanismo. Si las identidades de los grupos de aplicaciones de IIS (en tus servidores WFE y App) no usan cuentas administradas, deberás actualizar sus contraseñas manualmente en el Administrador de IIS en **cada servidor afectado** [24 (Tabla 3)]. Además, soluciones personalizadas, scripts de PowerShell, aplicaciones de terceros o incluso tareas programadas del sistema operativo podrían tener credenciales de cuentas de servicio **almacenadas directamente (hardcoded) o cacheadas**. Estos elementos no se actualizarán automáticamente y representan un riesgo significativo de ser pasados por alto, causando fallos inesperados en funcionalidades o automatizaciones. La complejidad de las cuentas no administradas subraya la ventaja de convertir a cuentas administradas siempre que sea posible.

Minimizar estos riesgos requiere una planificación meticulosa, identificación exhaustiva de todas las cuentas y sus dependencias, y seguir los procedimientos de actualización en el orden correcto, actualizando primero en Active Directory y luego en SharePoint.

Espero que esta información detallada te sea útil para comprender los riesgos involucrados.
