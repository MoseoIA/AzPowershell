...
.EXAMPLE
    ...
    .    \AddOwnerstoGpos_AzAD.ps1 -CsvPath .\ExampleGroupsOwners.csv
...
foreach ($row in $csvData) {
    ...
    try {
        ...
        $currentOwners = Get-AzureADGroupOwner -ObjectId $groupObject.ObjectId
        ...
        for ($i = 1; $i -le 5; $i++) {
            ...
            if (-not ([string]::IsNullOrEmpty($ownerUipn))) {
                try {
                    ...
                    if ($ownerObject.ObjectId -in $currentOwners.ObjectId) {
